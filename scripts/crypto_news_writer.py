...
# 돼 있었다 — 이 스크립트도 Gemini에게 원문 없이 기사 작성을 시키는
# 구조라 같은 사고가 날 수 있다.
try:
...
6. ⚠️ 숫자에 3자리마다 콤마(,)를 찍지 마세요. 시가총액·거래량처럼 큰
   ...
   쓰면 됩니다.
7. 비라틴 문자 애널리스트·거래소·기관명·인명은 정확한 한국어 표기로.
8. 분량: 500자 이상.
"""
...
# 대괄호([태그]) 형태만 인식해서, Gemini가 "가상자산동향, ..."처럼 평문으로
...
def enforce_title_prefix(title: str) -> str:
    """제목 앞에 [가상자산 동향] 태그를 강제 부착. 중복 부착 방지. (style_guard 공용화, 2026-09-09)"""
    return _sg_enforce_title_prefix(title, TITLE_PREFIX, "가상자산동향")
