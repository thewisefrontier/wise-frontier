...
def build_korea_today_report_kma(cities: list, local_now: datetime):
    ...
    lede = (
        f"{today_str} 기상청 단기예보 기준, 한국의 수도 {cap_name}은 오전 {cap['am_condition']}, "
        f"오후 {cap['pm_condition']}가 예상됩니다. 아침 최저기온은 {fmt_num(cap['tmin'])}°C, "
        f"낮 최고기온은 {fmt_num(cap['tmax'])}°C로 예보됐습니다."
    )

    rain_cities = [n for n, c, k in valid if (k.get("pm_precip") or k.get("am_precip") or 0) >= 60]
    if rain_cities:
        lede += f" {', '.join(rain_cities)} 등은 강수확률이 높아 우산을 챙기는 게 좋겠습니다."

    windy_cities = [n for n, c, k in valid if (k.get("wind_max") or 0) >= 40]
    if windy_cities:
        lede += f" {', '.join(windy_cities)} 등은 강풍에 유의해야 합니다."
...
# ── 기상청 중기예보 — 다음주 목·금(4~5일후) 보강용 ──────────
# 공식 활용가이드(기상청28_중기예보) 첨부자료에서 확인한 대표 지역코드.
KMA_MIDTERM_LAND_REGID = {
    "서울": "11B00000", "부산": "11H20000", "대구": "11H10000",
    "광주": "11F20000", "제주": "11G00000",
}
KMA_MIDTERM_TA_REGID = {
    "서울": "11B10101", "부산": "11H20201", "대구": "11H10701",
    "광주": "11F20501", "제주": "11G00201",
}


def _kma_midterm_tmfc(local_now: datetime) -> str:
    """중기예보 발표시각(06시, 18시) 중 지금 시점에서 가장 최근 발표를 고른다 (발표 후 10분 여유)"""
    hour, minute = local_now.hour, local_now.minute
    date = local_now.date()
    if hour > 18 or (hour == 18 and minute >= 10):
        chosen = 18
    elif hour > 6 or (hour == 6 and minute >= 10):
        chosen = 6
    else:
        date = date - timedelta(days=1)
        chosen = 18
...
def fetch_kma_midterm_land(name: str, tmfc: str, retries: int = 2):
    """중기육상예보(getMidLandFcst) 조회 — wf4Am~wf7Pm, rnSt4Am~rnSt7Pm 등 반환"""
    regid = KMA_MIDTERM_LAND_REGID.get(name)
    if not regid or not KMA_API_KEY:
        return None
    for attempt in range(retries):
        try:
            res = requests.get(
                ...
                params={
                    "serviceKey": KMA_API_KEY, "numOfRows": 10, "pageNo": 1, "dataType": "JSON",
...
def fetch_kma_midterm_ta(name: str, tmfc: str, retries: int = 2):
    """중기기온(getMidTa) 조회 — taMin4~taMax10 등 반환"""
    regid = KMA_MIDTERM_TA_REGID.get(name)
    if not regid or not KMA_API_KEY:
        return None
    for attempt in range(retries):
        try:
...
def format_korea_week_line(label: str, entries: list) -> str:
    """
    한국 다음주(월~금) 예보 — 단기예보(월~수) + 중기예보(목~금) 병합 결과를 도시당 한 줄로.
    entries: [(요일약칭, 상태설명 또는 None, 최고, 최저, 강수확률 또는 None), ...]
    """
    chain_parts = []
    tmax_list, tmin_list, rain_days = [], [], []
    any_data = False

    for wd, desc, tmax, tmin, precip in entries:
        if desc is None or tmax is None:
            chain_parts.append(f"{wd} 데이터없음")
            continue
        any_data = True
        chain_parts.append(f"{wd} {desc}({fmt_num(tmax)}°/{fmt_num(tmin)}°)")
        tmax_list.append((tmax, wd))
        tmin_list.append((tmin, wd))
...
def build_korea_weekly_report_kma(cities: list, local_now: datetime):
    """
    한국 전용 — 다음주(월~금) 예보. 일요일 아침 발행.
    월~수(1~3일후)는 단기예보(그글피까지 확장분), 목~금(4~5일후)은 중기예보로 채운다.
    """
    today_str = local_now.strftime("%Y년 %m월 %d일") + f"({WEEKDAY_KO[local_now.weekday()]})"

    short_dates = [(local_now + timedelta(days=i)).strftime("%Y%m%d") for i in (1, 2, 3)]
    short_wds = [WEEKDAY_KO[(local_now + timedelta(days=i)).weekday()] for i in (1, 2, 3)]
    mid_tmfc = _kma_midterm_tmfc(local_now)
    thu_wd = WEEKDAY_KO[(local_now + timedelta(days=4)).weekday()]
    fri_wd = WEEKDAY_KO[(local_now + timedelta(days=5)).weekday()]

    fetched = _kma_fetch_cities(cities, local_now)  # 단기예보 (월~수)
    ...
    for name, is_capital, by_date in fetched:
        ...
        for d, wd in zip(short_dates, short_wds):
            info = by_date.get(d) if by_date else None
            if info:
                desc = info["am_condition"] if info["am_condition"] == info["pm_condition"] \
                    else f"{info['am_condition']}→{info['pm_condition']}"
                precip = max(info.get("am_precip") or 0, info.get("pm_precip") or 0)
                entries.append((wd, desc, info["tmax"], info["tmin"], precip))
            else:
                entries.append((wd, None, None, None, None))

        land = fetch_kma_midterm_land(name, mid_tmfc)
        ta = fetch_kma_midterm_ta(name, mid_tmfc)
        for day_num, wd_label in [(4, thu_wd), (5, fri_wd)]:
            if land and ta and land.get(f"wf{day_num}Am"):
                am, pm = land.get(f"wf{day_num}Am"), land.get(f"wf{day_num}Pm")
                desc = am if am == pm else f"{am}→{pm}"
                try:
                    tmax = float(ta.get(f"taMax{day_num}"))
                    tmin = float(ta.get(f"taMin{day_num}"))
                except (TypeError, ValueError):
                    tmax = tmin = None
                rn_am = land.get(f"rnSt{day_num}Am")
                rn_pm = land.get(f"rnSt{day_num}Pm")
                try:
                    precip = max(int(rn_am or 0), int(rn_pm or 0))
                except (TypeError, ValueError):
                    precip = None
                entries.append((wd_label, desc, tmax, tmin, precip))
            else:
                entries.append((wd_label, None, None, None, None))
        time.sleep(1)

        line = format_korea_week_line(label, entries)
        lines.append(line)
    ...
    summary = f"{today_str} 발표된 한국 다음주(월~금) 날씨 예보입니다."
    if cap_entries:
        valid_days = [e for e in cap_entries if e[1] is not None]
        if valid_days:
            tmax_all = [e[2] for e in valid_days]
            tmin_all = [e[3] for e in valid_days]
            rain_days = [e[0] for e in valid_days if (e[4] or 0) >= 50]
            summary = (
                f"{today_str} 발표된 다음주(월~금) 전망입니다. 한국의 수도 {cap_name}은 "
                f"이번 주 최고 {fmt_num(max(tmax_all))}°C, 최저 {fmt_num(min(tmin_all))}°C 사이를 오갈 것으로 보입니다."
            )
            if rain_days:
                summary += f" {'·'.join(rain_days)}요일에는 비 소식이 있으니 참고하시기 바랍니다."
            else:
                summary += " 당분간 비 소식 없이 대체로 맑은 날씨가 이어질 전망입니다."
            summary += " (월~수: 기상청 단기예보, 목~금: 기상청 중기예보 기준)"
