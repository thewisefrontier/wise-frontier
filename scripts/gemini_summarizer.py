...
def fetch_background_context(query: str) -> str:
    ...
    #
    # ⚠️ 2026-09-10 실사고(id=151545): "인물의 이력·직함처럼 시간이 지나도
    # 안 바뀌는 사실"이라고 직함을 안전 카테고리에 넣어놨었는데, 정확히
    # 반대다 — 이력(과거 행적)은 안 바뀌지만 현재 직함은 인사가 날 때마다
    # 바뀌는 정보다. 이 문구 때문에 NVIDIA가 "에드 밀리반드가 현 영국
    # 외무장관"인 실제 사건에 "데이비드 밀리반드(2007~2010년 당시 외무장관,
    # 형제 관계)"를 확신 있게 답해 그대로 기사에 실렸다(사용자 지적: "데이비드
    # 밀리밴드는 국제구조위원회 총재네. 장관이 아닌 거 아냐?"). 실시간 검색이
    # 없는 NVIDIA는 애초에 "현재 누가 그 자리에 있는지" 답할 자격이 없다 —
    # 직함을 "모름" 대상으로 명시해 이 경로 자체를 차단한다.
    #
    # ⚠️ 첫 수정 때는 이 "현재 직함 금지"를 base_prompt(검색 그라운딩 경로와
    # 공유)에 넣었었는데, 사용자 지적: "절대 쓰지 말 것으로 고정해버리면
    # 검색 결과가 기억하고 있는 현재 직함/직책이 나와도 안 쓰게 되는거
    # 아냐?" — 맞는 지적이라 되돌림. 검색 그라운딩(2차)은 실시간 검색
    # 결과로 실제 확인이 가능하므로 여기(NVIDIA 전용)에만 금지를 건다.
    nvidia_prompt = (
        base_prompt + "\n\n주의: 당신은 실시간 인터넷 조회 없이 학습된 지식만으로 "
        "답합니다. 인물의 과거 행적·역사적 사실처럼 시간이 지나도 안 바뀌는 사실만 "
        "확신 있게 쓰세요. 주가·환율·사망자 수·현재 상황은 물론, 인물의 현재 "
        "직함·직책도 인사가 나면 바뀌므로 절대 확신하지 말고 언급 자체를 하지 "
        "마세요. 그런 내용밖에 없거나 확인되지 않은 추측은 쓰지 말고 '모름'이라고만 "
        "답하세요."
    )
    try:
