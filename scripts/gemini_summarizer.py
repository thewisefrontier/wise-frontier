...
def fetch_background_context(query: str) -> str:
    ...
    base_prompt = (
        ...
        "우선하세요. ⚠️ 인물의 현재 직함·직책(예: 현재 장관인지, 현재 어느 자리에 "
        "있는지)은 절대 쓰지 마세요 — 사람이 바뀌어도 이름이 같거나 비슷한 경우가 "
        "많아(형제·동명이인 등) 최신 재직 여부를 착각하기 쉽습니다. 본문에 이미 "
        "있는 관련 기사만 현재 직함의 근거로 삼습니다.\n\n"
        f"소재: {query}"
    )
...
def _verify_no_hallucination(text: str, call_fn) -> str:
    ...
    check_prompt = (
        ...
        "위험한 문장이 있으면 그 문장만 전부 제거하세요. 인물의 현재 직함·직책을 "
        "언급하는 문장은 항상 위험한 문장으로 취급해 제거하세요(재직 여부는 계속"
        "바뀌고, 동명이인·형제 등과 착각하기도 쉽습니다 — 실사고: 데이비드 "
        "밀리반드를 현 영국 외무장관으로 오인, 실제로는 2007~2010년 당시 직함이고 "
        "그 자리는 현재 동생 에드 밀리반드). 인물의 과거 행적·역사적 배경처럼 "
        "시간이 지나도 안 바뀌는 안전한 사실은 그대로 남기세요. 위험한 문장이 "
        "하나도 없으면 원문 그대로 반환하고, 안전한 내용이 전혀 남지 않으면 정확히 "
        "'없음'이라고만 답하세요. 다른 설명 없이 결과만 출력하세요.\n\n"
        f"{text}"
    )
    try:
...
- 반드시 하나의 토픽만 다루세요.
- [배경 참고자료]가 있다면 인물의 과거 행적·이력, 사건의 경위·파급 영향처럼 위 관련 기사에 없는 사실관계를 보완하는 데 쓰세요. 다만 이 자료도 검색 결과이므로 관련 기사와 모순되면 관련 기사를 우선하고, 확신이 낮은 내용은 쓰지 마세요. ⚠️ 인물의 현재 직함·직책은 [배경 참고자료]에 있더라도 절대 쓰지 마세요 — 위 [관련 기사]에 명시적으로 나온 직함만 쓰고, 관련 기사에도 없으면 직함 없이 이름만 쓰세요.
