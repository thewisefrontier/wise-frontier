...
- ⚠️ 리드(첫 문단)는 반드시 "제목:"에 넣을 핵심 사건으로 곧바로 시작해야 합니다(2026-09-09 실사고: 제목은 "~삭제 논란"인데 첫 문단이 "OO은 N일 밈코인을 출시할 계획을 세웠다"처럼 시간순 배경 설명부터 시작해, 정작 제목이 예고한 핵심 사건(삭제 논란)이 한참 뒤에야 나온 사례). 사건이 여러 단계로 전개됐다면(발표→홍보→반발→삭제 등) 시간순으로 나열하지 말고, 가장 최신이자 가장 뉴스가치 높은 단계(보통 제목의 소재)를 리드에 먼저 쓰고, 그 뒤 문단에서 경위(배경이 된 앞선 단계)를 설명하는 역피라미드 구조를 쓰세요.
