...
def _pub_day_label(raw) -> str:
    ...
    ⚠️ UTC 기준이라 현지시간과 최대 하루 어긋날 수 있다. 그럼에도 주입하는 이유는
    country 컬럼이 원본 기사의 55.9%에서 비어 국가별 보정이 불가능하고(2026-08-05 실측),
    아무 날짜도 주지 않으면 Gemini가 요일로 도피하거나 날짜를 지어내기 때문이다.
    원문 본문에 날짜가 있으면 그쪽이 우선이라는 규칙은 프롬프트에 명시돼 있다.
    """
    s = str(raw or "")[:10]
...
- 반드시 하나의 토픽만 다루세요.
- 본문은 부가가치 문단을 포함해 3개 문단 이상으로 충분히 작성하세요. 일반적인 스트레이트 기사의 표준 분량은 200자 원고지 10매, 즉 대략 2,000자 정도입니다(2026-08-26 사용자 지적: "죄다 단신급의 기사" → "적어도 기사 하나마다 분량이 2천자 정도는 됐으면", "물론 어거지로 늘리는건 아니고 가능한선에서") — 700자는 최소 하한선이지 목표치가 아니니 그 근처에서 서둘러 마무리하지 마세요. 배경·경과·전망(또는 파급 효과)을 각각 다루어 분량을 채우세요. 관련 기사에 나온 내용이 부족하면 배경 설명이나 맥락으로 보완하되, 소스에 없는 내용을 지어내 채우지는 마세요 — 소스가 짧은 단신이면 억지로 2,000자를 채우지 말고 가능한 만큼만 쓰세요.
- 문단을 나눌 때는 반드시 빈 줄(줄바꿈 2번)로 구분하세요. 한 문단에 모든 문장을 붙여 쓰지 마세요.
- 수치, 인명, 날짜, 기관명 등 구체적 팩트를 최대한 살리세요.
...
- 본문은 부가가치 문단을 포함해 3개 문단 이상으로 작성하세요. 일반적인 스트레이트 기사의 표준 분량은 200자 원고지 10매, 즉 대략 2,000자 정도입니다 — 700자는 최소 하한선이지 목표치가 아니니 그 근처에서 서둘러 마무리하지 마세요. 배경·경과·의미(또는 파급 효과)를 각각 풀어서 서술하되, 소스가 짧은 단신이면 억지로 채우지 말고 가능한 만큼만 쓰세요.
- 문단을 나눌 때는 반드시 빈 줄(줄바꿈 2번)로 구분하세요. 한 문단에 모든 문장을 붙여 쓰지 마세요.
