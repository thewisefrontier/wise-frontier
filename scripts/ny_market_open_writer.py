...
2. 구조 — 아래 소제목을 본문에 그대로 포함하고 소제목 앞에는 빈 줄을 하나씩 두세요:
   ① 리드: ⚠️ 반드시 미국 선물지수(S&P500·나스닥·다우 선물) 방향으로 시작하세요.
      "{today.day}일(현지시간) 뉴욕증시 개장을 앞두고"처럼 개장 전 시점임을
      분명히 하고, 검색으로 찾은 실제 뉴스 기반으로 왜 그런 방향인지 반영하세요.
      선물이 엇갈리면(혼조) 그 사실 자체를 리드로 쓰세요.
   ② [간밤 해외증시] 아시아·유럽 주요 증시의 마감 상황을 변동폭 상위 2~3개
      중심으로, 실제 이유(검색으로 찾은 뉴스)와 함께 서술. 거래소명 포함.
   ③ [오늘의 관전 포인트] 검색으로 찾은 그날 예정된 경제지표 발표·연준 인사
      발언·주요 기업 실적 발표 등 개장 후 시장을 움직일 만한 일정을 서술.
      확인 안 되는 내용은 지어내지 말고, 못 찾으면 이 문단은 생략하세요.
3. ⚠️ 날짜: 반드시 리드(①) 문단에 "{today.day}일(현지시간)" 형식으로 날짜를
