...
아래 한국석유공사 오피넷 전국 평균 유가 데이터를 바탕으로 뉴스 기사를 작성하세요.

[유가 데이터] (출처: 한국석유공사 오피넷)
- 공개일: {pdate.day}일 (오피넷이 이 날짜로 발표한 값 — 오피넷 공지에 따르면
...
2. 구조:
   ① 리드: "한국석유공사 오피넷이 {pdate.day}일 공개한 자료에 따르면 국내
      주유소 휘발유 평균 판매가격은 리터당 {gasoline_price}원으로 집계됐다." 형태로,
      "공개한 자료에 따르면"처럼 발표 시점을 명시하는 표현을 쓰세요. "{pdate.day}일
      기준", "이날", "오늘 판매가"처럼 그 날짜의 실시간·당일 가격인 것처럼
      단정하는 표현은 쓰지 마세요(위 [유가 데이터] 설명 참고 — 공개일과 실제 판매 시점이
      하루 어긋날 수 있음).
   ② 경유·LPG 등 다른 유종 가격·전일 대비 수치
   {bg_instruction}
   ④ 소비자·자영업자(운수업 등) 체감 영향
3. 날짜는 "{pdate.day}일 공개" 형식으로 발표 시점임을 명시. "오늘", "현재",
   절대연도 금지. "{pdate.day}일 기준"처럼 단정하는 표현도 금지.
4. 출처 표기: 리드(①)에서만 "한국석유공사 오피넷"으로 기관명 전체를 밝히고, 본문
   다른 곳에서 다시 출처를 언급할 때는 "오피넷에 따르면"처럼 "오피넷"만 쓰세요.
   "오피넷(한국석유공사)"를 본문에 두 번 이상 반복하지 마세요.
5. 분량: 500자 이상.
