...
[3줄요약 작성 규칙]
기사 핵심을 정확히 3줄로 요약하세요. 각 줄은 "\\n"으로 구분된 완결된 문장이며, 각 줄은 40자 내외로 간결하게 쓰세요. 본문 내용을 그대로 자르지 말고, 핵심만 압축하세요.
...
5. 프론티어 마켓 파급 효과: 기사 주체가 선진국(미국·유럽·일본 등)이더라도, 대형 재난·공급망 충격·자원가격 변동 등은 아프리카·동남아시아·중앙아시아 등 프론티어 마켓에도 실질적 영향을 미칩니다(예: 관광 의존 경제 타격, 원자재·농산물 가격 변동, 보험·재보험 시장, 이주노동자 송금 감소 등). 그런 연결고리가 실제로 있으면 구체적으로 짚어주세요. 없으면 억지로 만들지 마세요.
"~에 영향을 미칠 것으로 보인다", "주목할 필요가 있다", "지켜봐야 한다" 같은 막연한 상투 문구는 절대 금지 — 구체적 인과관계·수치·비교를 담으세요. 이미 본문에서 서술한 사실을 다른 말로 반복하는 것은 부가가치가 아닙니다.
