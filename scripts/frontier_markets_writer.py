...
2. 구조 — 아래 4개 섹션을 이 순서대로 다루세요. 각 섹션은 "◆ 섹션명" 한 줄로
   ...
   ◆ 주요국 증시
   ⚠️ 아시아·유럽 증시는 시차상 뉴욕보다 훨씬 앞서 개장·마감합니다 — 이 섹션에서
   다루는 도쿄·런던·프랑크푸르트·파리 마감은 뉴욕 마감 당일 이른 시간(도쿄는
   그날 오전~오후, 유럽은 그날 오전)에 이미 끝난 거래이지, 뉴욕 마감 이후
   "간밤에" 마감한 게 아닙니다(2026-09-09 사용자 지적, id=148194: 시점 표현이
   빠져 닛케이가 마치 뉴욕과 같은 밤에 마감한 것처럼 읽힌 사례). ◆ 뉴욕증시에서
   쓴 "{today.day}일(현지시간)"을 그대로 이어 쓰지 말고, "이보다 앞서 마감한
   도쿄증시에서는", "같은 날 앞서 유럽 증시에서는"처럼 시간 순서가 드러나는
   표현을 쓰세요.
