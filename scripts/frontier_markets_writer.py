...
# ⚠️ 2026-09-18 사용자 지시: "그냥 해외 증시 종합기사에서 일본 닛케이를
# 빼자" — "시간대 문제 때문에 자꾸 오류가 발생하는데" (도쿄는 뉴욕보다
# 훨씬 앞서 마감해 "같은 날"류 표현이 반복적으로 꼬이는 사고, id=148194
# 이후에도 id=194024에서 재발). 프롬프트 지시만으로 계속 재발해 닛케이는
# 이 종합기사에서 완전히 제외 — 별도 스크립트(jp_market_writer.py)로 분리.
MAJOR_INDICES = [
    ("미국", "S&P500", "^GSPC", "뉴욕증시"),
    ("미국", "나스닥종합", "^IXIC", "뉴욕증시"),
    ("미국", "다우존스", "^DJI", "뉴욕증시"),
...
2. 구조 — 아래 4개 섹션을 이 순서대로 다루세요. 각 섹션은 "◆ 섹션명" 한 줄로
   ...
   ◆ 주요국 증시
   ⚠️ 유럽 증시는 시차상 뉴욕보다 훨씬 앞서 개장·마감합니다 — 이 섹션에서
   다루는 런던·프랑크푸르트·파리 마감은 뉴욕 마감 당일 이른 시간(그날 오전)에
   이미 끝난 거래이지, 뉴욕 마감 이후 "간밤에" 마감한 게 아닙니다(2026-09-09
   사용자 지적, id=148194). ◆ 뉴욕증시에서 쓴 "{today.day}일(현지시간)"을
   그대로 이어 쓰지 말고, "같은 날 앞서 유럽 증시에서는"처럼 시간 순서가
   드러나는 표현을 쓰세요.
   뉴욕 외 변동폭 상위 지수 2~3개(영국·독일·프랑스 등)를, 그 지수가 움직인
   실제 이유(검색으로 찾은 뉴스 — 연준 금리 발표, 주요 기업 실적, 경제 지표 등)와
   함께 서술. 거래소명 포함. 뉴욕 지수 상세 수치가 ◆ 뉴욕증시 섹션에서 다 안
   다뤄졌으면 여기서 마저 다루세요.
   ⚠️ 일본 닛케이는 이 종합기사에서 다루지 않습니다(2026-09-18부터 별도
   기사로 분리 — jp_market_writer.py). MAJOR_INDICES에 없으므로 데이터 자체가
   안 넘어오지만, 검색으로 알게 되더라도 닛케이를 언급하지 마세요.
...
def enforce_title_prefix(title: str) -> str:
    """제목 앞에 [글로벌 마켓 동향] 태그를 강제 부착. 중복 부착 방지. (style_guard 공용화, 2026-09-09)"""
    return _sg_enforce_title_prefix(title, TITLE_PREFIX, "글로벌마켓동향", particles=("이", "은"))
