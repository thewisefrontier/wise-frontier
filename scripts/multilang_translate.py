...
# 2026-09-09: 애초 10건(언어 4개=40콜) 그대로 유지 — 타임아웃 사고 당시
# 실제 로그(사용자 확인)에 이 스텝은 안 걸려 있었다(걸린 건 GDELT/국내뉴스
# 수집 쪽의 crawl_full_text() 비용). 오히려 너무 낮추면(5건) 국내뉴스
# 수집 백로그(현재 66건) 소진이 느려진다는 지적(사용자: "번역하고 그러면
# 시간이 꽤 걸릴 것 같은데") — Gemini+NVIDIA 호출만이라 크롤링보다 훨씬
