"""
digest_rules 프롬프트를 Supabase prompts 테이블에 등록 (최초 1회 실행)
"""
import os
import requests
...
def init():
    ...
    res = requests.post(
        ...
        headers=_headers(),
        json={"name": "digest_rules", "content": DIGEST_RULES, "version": 1, "is_active": True},
    )
    print(f"✅ digest_rules 등록" if res.status_code in (200,201,204) else f"❌ {res.text[:150]}")
