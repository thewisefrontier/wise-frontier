...
# writer_rules(DB 프롬프트) 로드 실패 시에만 쓰는 폴백 — 예전엔 이 텍스트가
# build_cluster_prompt()(클러스터 경로)와 단독 기사 생성 경로 두 곳에 각자
# 따로 정의돼 있었고, 단독 기사 쪽 사본이 요약이 아니라 기사 작성 지시·
# 요일/보도일 날짜 규칙·원어 괄호 병기 순서 등 여러 규칙이 빠진 축약판이었다
# (2026-09-09 사용자 지시: "gemini_writer.py도 같은 방식으로 통합해줘" —
# gemini_summarizer.py의 TREND_LEAD_STRUCTURE_RULES 공용화와 동일한 이유).
# 평소엔 writer_rules DB 프롬프트가 정상 로드되므로 두 곳 다 실제로는 같은
# 내용을 쓰지만, DB 로드가 실패하는 드문 경우에만 이 차이가 드러나 조용히
# 더 부실한 기사가 나갈 수 있었다 — 상수 하나로 통일해 그 위험을 없앤다.
FALLBACK_RULES = """[주의사항]
...
- 각 원문 제목 옆에 "(보도 M월 D일)"이 붙어 있을 수 있습니다. 원문 본문에 숫자로 된 구체적 날짜가 명시돼 있으면 그 날짜가 우선입니다. ⚠️ 원문에 "on Monday"처럼 요일만 있고 숫자 날짜가 없으면 "날짜가 명시된 것"으로 치지 말고 보도일로 넘어가세요(2026-09-08 실사고 — 요일만 있는 원문을 "날짜 있음"으로 오판해 보도일 대체를 안 쓰고, 요일 단독 표기도 금지라 결국 기사 전체에 날짜가 하나도 안 들어간 사고). 숫자 날짜가 없을 때만 보도일을 사건 시점으로 보고 "D일(현지시간)" 형식으로 쓰세요. 보도일 표기가 없고 원문에도 숫자 날짜가 없으면 그때만 날짜를 쓰지 마세요.
...
# "공용모듈이 필요한 시스템이 더 있는지 점검해줘"). fabrication_guard.py로
...
- ⚠️ "새로운 사실"은 표현이 다르다고 인정되는 게 아니라, 핵심 사실관계 자체가 달라진 경우만입니다(예: 조치가 실제로 해제/변경됐다, 수치가 갱신됐다, 새로운 조치가 발표됐다). 후속 정보가 이미 반영된 업데이트와 같은 핵심 사실(예: 이미 "비상사태가 해제됐다"고 반영돼 있는데 다른 통신사가 같은 해제 소식을 다르게 표현했을 뿐인 경우)을 다른 문장·다른 통신사·약간 다른 수준의 디테일로 재확인하는 것뿐이라면, 그건 새 정보가 아닙니다 — 세부 문구가 다르다고 새로 쓰지 말고 "업데이트 불필요"로 답하세요.
- 새로 확인된 사실이 없으면 정확히 "업데이트 불필요" 한 줄만 출력하세요. 분량을 채우기 위해 이미 나온 사실을 다른 각도·다른 수치 표현으로 다시 쓰지 마세요.
...
- 날짜는 후속 정보에 명시된 "N일(현지시간)" 형식만 쓰세요. 날짜 근거가 없으면 생략하고 추측하지 마세요. 단, "(현지시간)"은 이번에 새로 쓰는 이 업데이트 문단 안에서 첫 날짜 표기에만 한 번 붙이고, 같은 문단 안에서 또 다른 날짜를 언급할 때는 "(현지시간)" 없이 "N일"만 쓰세요.
