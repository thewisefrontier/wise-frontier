...
export async function onRequestGet(context) {
  ...
  // 정적 셸 (env.ASSETS = Pages 정적 자산 바인딩)
  const shellReq = new Request(new URL('/article.html', url.origin), request);
  const shell = await env.ASSETS.fetch(shellReq);

  // id 없으면 셸 그대로 반환(클라가 "기사를 찾을 수 없습니다" 처리)
  if (!id) return shell;

  // Supabase 서버 조회 (article.html과 동일 쿼리, is_published=true 1건)
  let a = null;
  try {
  ...
  return new Response(res.body, {
    ...
    headers: {
      ...
      // 크롤러/공유용 짧은 캐시. 사용자는 클라 재렌더로 최신 반영.
