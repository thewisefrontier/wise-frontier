...
function buildWrapperHtml(a) {
  ...
  // 업데이트 기록 공개 필터 — 내부 운영 기록(트렌드 감지 경로, 자동 중복정리, 표기·문체
  // 교정, 복수주제 파킹, 관리자 수동 정리 사유 등)은 독자에게 노출하지 않는다. 첫 항목은
  // '최초 게시', 이후는 이 블랙리스트에 안 걸리는 것만 '내용 업데이트'로 일반화해 표시한다.
  // 원문 전체는 admin.html 기사 편집 화면에서 확인한다.
  // ※ articles.json은 export_articles.py에서 이미 정화되지만, Supabase 직접 조회 경로가
  //   원문을 그대로 받으므로 렌더 단계에도 같은 필터를 둔다.
  // ⚠️ 실사고(2026-08-09): 예전엔 화이트리스트 방식이었는데, 실제 업데이트 경로들이 쓰는
  //    노트 문구("후속 정보 추가", generate_update_note()의 자유 문장 등)와 화이트리스트가
  //    하나도 안 맞아 진짜 업데이트가 있어도 로그 박스에 항상 안 보였다. 블랙리스트로 뒤집음.
  var INTERNAL_ONLY_NOTE_RE = /실시간 트렌드 감지|자동 중복정리|음역 자동 교정|문자셋 이탈|복수주제 분리 파킹|수동 정리/;
  function publicUpdateLog(log) {
